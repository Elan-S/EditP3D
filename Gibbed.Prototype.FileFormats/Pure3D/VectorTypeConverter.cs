    ...
    public class VectorTypeConverter : TypeConverter
    {
                                         ...
                                         Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                var rc = value as RotationCompressed;
                if (rc != null)
                {
                    return String.Format(CultureInfo.InvariantCulture,
                                         "Rotation(X={0:0.000000}, Y={1:0.000000}, Z={2:0.000000}",
                                         (float)(rc.X / 65535f) * (180.0 / Math.PI),
                                         (float)(rc.Y / 65535f) * (180.0 / Math.PI),
                                         (float)(rc.Z / 65535f) * (180.0 / Math.PI));
                }
                var v3c = value as Vector3Compressed;
                if (v3c != null)
                {
                    return String.Format(CultureInfo.InvariantCulture,
                                         "Vector3(X={0:0.000000}, Y={1:0.000000}, Z={2:0.000000})",
                                                                   ...
                                                                   Attribute[] attributes)
        {
            ...
            else if (value is RotationCompressed)
            {
                properties = new[]
                {
                    new Descriptor(typeof(RotationCompressed), typeof(float), "X"),
                    new Descriptor(typeof(RotationCompressed), typeof(float), "Y"),
                    new Descriptor(typeof(RotationCompressed), typeof(float), "Z"),
                };
            }
            else if (value is Vector3Compressed)
            {
                properties = new[]
                {
                    new Descriptor(typeof(Vector3Compressed), typeof(float), "X"),
                    new Descriptor(typeof(Vector3Compressed), typeof(float), "Y"),
                    new Descriptor(typeof(Vector3Compressed), typeof(float), "Z"),
                };
            }
        ...
        private class Descriptor : SimplePropertyDescriptor
        {
            ...
            public override object GetValue(object instance)
            {
                ...
                var rotationcompressed = instance as RotationCompressed;
                if (rotationcompressed != null)
                {
                    switch (this._Name)
                    {
                        case "X":
                            {
                                return rotationcompressed.X.ToString("0.000000", CultureInfo.InvariantCulture);
                            }

                        case "Y":
                            {
                                return rotationcompressed.Y.ToString("0.000000", CultureInfo.InvariantCulture);
                            }

                        case "Z":
                            {
                                return rotationcompressed.Z.ToString("0.000000", CultureInfo.InvariantCulture);
                            }
                ...
                var vector3compressed = instance as Vector3Compressed;
                if (vector3compressed != null)
                {
                    switch (this._Name)
                    {
                        case "X":
                            {
                                return vector3compressed.X.ToString("0.000000", CultureInfo.InvariantCulture);
                            }

                        case "Y":
                            {
                                return vector3compressed.Y.ToString("0.000000", CultureInfo.InvariantCulture);
                            }

                        case "Z":
                            {
                                return vector3compressed.Z.ToString("0.000000", CultureInfo.InvariantCulture);
                            }
