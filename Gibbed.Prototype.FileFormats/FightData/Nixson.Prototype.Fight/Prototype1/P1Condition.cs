...
namespace Nixson.Prototype.Fight.Prototype1
{
	public class P1Condition : BaseCondition
	{
		...
		{
			BaseCondition baseCondition = Factory<BaseCondition, KnownConditionAttribute>.Build(PrototypeGame.P1, hash);
			...
			baseCondition.DeserializeProperties(PrototypeGame.P1, input, endianess);
			input.ReadValueU64();
		...
		public static List<BaseCondition> DeserializeBaseConditions(Stream input, Endian endianess)
		{
			...
			for (;;)
			{
				...
				list.Add(P1Condition.DeserializeBaseCondition(input, endianess, num));
			}
			input.Position -= 8L;
			return list;
		}
